...
public:
    ...
    bool knnSearch(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], int number,
                   std::function<bool(const DATATYPE &)> callback) const;

    bool knnSearch(DATATYPE &target, int number, std::function<bool(const DATATYPE &)> callback) const;

    bool knnSearch(Node *a_node, DATATYPE &target, int number, std::function<bool(const DATATYPE &)> callback) const;

    Node *getNode(DATATYPE target) const {
        assert(target);
        return getNode(m_root, target);
    }
    ...
    Node *getNode(Node *a_node, DATATYPE target) const {
        ...
        while (!st.empty()) {
            ...
            if (cur->IsInternalNode()) {
                for (int index = 0; index < cur->m_count; ++index) {
//                     return getNode(a_node->m_branch[index].m_child, target);
                    st.push(cur->m_branch[index].m_child);
                }
            } else {
                //如果是数据结点  进行判断  注意数据结点是没用叶子结点的
                for (int index = 0; index < cur->m_count; ++index) {
                    if (target == cur->m_branch[index].m_data) {
                        return cur;
                    }
                }
            }
    ...
    std::vector<DATATYPE> getSameBranchData(DATATYPE &target) const {
        Node *node = getNode(target);
        assert(node);
        std::vector<DATATYPE> res;
        for (int index = 0; index < node->m_count; ++index) {
            res.push_back(node->m_branch[index].m_data);
        }
        return res;
    }
    ...
    private:
        ...
        enum {
            MAX_STACK = 32
        }; //  std::max stack size. Allows almost n^32 where n is number of branches in node
...
template<class DATATYPE, class ELEMTYPE, int NUMDIMS, class ELEMTYPEREAL, int TMAXNODES, int TMINNODES>
RTree<DATATYPE, ELEMTYPE, NUMDIMS, ELEMTYPEREAL, TMAXNODES, TMINNODES>::RTree(const RTree &other) : RTree() {
...
template<class DATATYPE, class ELEMTYPE, int NUMDIMS, class ELEMTYPEREAL, int TMAXNODES, int TMINNODES>
void RTree<DATATYPE, ELEMTYPE, NUMDIMS, ELEMTYPEREAL, TMAXNODES, TMINNODES>::CountRec(Node *a_node, int &a_count) {
...
template<class DATATYPE, class ELEMTYPE, int NUMDIMS, class ELEMTYPEREAL, int TMAXNODES, int TMINNODES>
void RTree<DATATYPE, ELEMTYPE, NUMDIMS, ELEMTYPEREAL, TMAXNODES, TMINNODES>::CopyRec(Node *current, Node *other) {
...
template<class DATATYPE, class ELEMTYPE, int NUMDIMS, class ELEMTYPEREAL, int TMAXNODES, int TMINNODES>
typename RTree<DATATYPE, ELEMTYPE, NUMDIMS, ELEMTYPEREAL, TMAXNODES, TMINNODES>::Node *
RTree<DATATYPE, ELEMTYPE, NUMDIMS, ELEMTYPEREAL, TMAXNODES, TMINNODES>::AllocNode() {
    Node *newNode;
#ifdef RTREE_DONT_USE_MEMPOOLS
    newNode = new Node;
#else // RTREE_DONT_USE_MEMPOOLS
...
template<class DATATYPE, class ELEMTYPE, int NUMDIMS, class ELEMTYPEREAL, int TMAXNODES, int TMINNODES>
typename RTree<DATATYPE, ELEMTYPE, NUMDIMS, ELEMTYPEREAL, TMAXNODES, TMINNODES>::ListNode *
RTree<DATATYPE, ELEMTYPE, NUMDIMS, ELEMTYPEREAL, TMAXNODES, TMINNODES>::AllocListNode() {
#ifdef RTREE_DONT_USE_MEMPOOLS
    return new ListNode;
#else // RTREE_DONT_USE_MEMPOOLS
    // EXAMPLE
...
template<class DATATYPE, class ELEMTYPE, int NUMDIMS, class ELEMTYPEREAL, int TMAXNODES, int TMINNODES>
typename RTree<DATATYPE, ELEMTYPE, NUMDIMS, ELEMTYPEREAL, TMAXNODES, TMINNODES>::Rect
RTree<DATATYPE, ELEMTYPE, NUMDIMS, ELEMTYPEREAL, TMAXNODES, TMINNODES>::NodeCover(Node *a_node) {
    assert(a_node);
...
template<class DATATYPE, class ELEMTYPE, int NUMDIMS, class ELEMTYPEREAL, int TMAXNODES, int TMINNODES>
bool
RTree<DATATYPE, ELEMTYPE, NUMDIMS, ELEMTYPEREAL, TMAXNODES, TMINNODES>::AddBranch(const Branch *a_branch, Node *a_node,
                                                                                  Node **a_newNode) {
    assert(a_branch);
    assert(a_node);
...
template<class DATATYPE, class ELEMTYPE, int NUMDIMS, class ELEMTYPEREAL, int TMAXNODES, int TMINNODES>
void
RTree<DATATYPE, ELEMTYPE, NUMDIMS, ELEMTYPEREAL, TMAXNODES, TMINNODES>::DisconnectBranch(Node *a_node, int a_index) {
...
template<class DATATYPE, class ELEMTYPE, int NUMDIMS, class ELEMTYPEREAL, int TMAXNODES, int TMINNODES>
int
RTree<DATATYPE, ELEMTYPE, NUMDIMS, ELEMTYPEREAL, TMAXNODES, TMINNODES>::PickBranch(const Rect *a_rect, Node *a_node) {
...
template<class DATATYPE, class ELEMTYPE, int NUMDIMS, class ELEMTYPEREAL, int TMAXNODES, int TMINNODES>
typename RTree<DATATYPE, ELEMTYPE, NUMDIMS, ELEMTYPEREAL, TMAXNODES, TMINNODES>::Rect
RTree<DATATYPE, ELEMTYPE, NUMDIMS, ELEMTYPEREAL, TMAXNODES, TMINNODES>::CombineRect(const Rect *a_rectA,
                                                                                    const Rect *a_rectB) {
...
template<class DATATYPE, class ELEMTYPE, int NUMDIMS, class ELEMTYPEREAL, int TMAXNODES, int TMINNODES>
void
RTree<DATATYPE, ELEMTYPE, NUMDIMS, ELEMTYPEREAL, TMAXNODES, TMINNODES>::SplitNode(Node *a_node, const Branch *a_branch,
                                                                                  Node **a_newNode) {
    assert(a_node);
    assert(a_branch);
...
template<class DATATYPE, class ELEMTYPE, int NUMDIMS, class ELEMTYPEREAL, int TMAXNODES, int TMINNODES>
void RTree<DATATYPE, ELEMTYPE, NUMDIMS, ELEMTYPEREAL, TMAXNODES, TMINNODES>::LoadNodes(Node *a_nodeA, Node *a_nodeB,
                                                                                       PartitionVars *a_parVars) {
    assert(a_nodeA);
    assert(a_nodeB);
    assert(a_parVars);
...
template<class DATATYPE, class ELEMTYPE, int NUMDIMS, class ELEMTYPEREAL, int TMAXNODES, int TMINNODES>
void RTree<DATATYPE, ELEMTYPE, NUMDIMS, ELEMTYPEREAL, TMAXNODES, TMINNODES>::Classify(int a_index, int a_group,
                                                                                      PartitionVars *a_parVars) {
    assert(a_parVars);
...
template<class DATATYPE, class ELEMTYPE, int NUMDIMS, class ELEMTYPEREAL, int TMAXNODES, int TMINNODES>
bool
RTree<DATATYPE, ELEMTYPE, NUMDIMS, ELEMTYPEREAL, TMAXNODES, TMINNODES>::RemoveRect(Rect *a_rect, const DATATYPE &a_id,
                                                                                   Node **a_root) {
...
template<class DATATYPE, class ELEMTYPE, int NUMDIMS, class ELEMTYPEREAL, int TMAXNODES, int TMINNODES>
bool
RTree<DATATYPE, ELEMTYPE, NUMDIMS, ELEMTYPEREAL, TMAXNODES, TMINNODES>::Overlap(Rect *a_rectA, Rect *a_rectB) const {
    assert(a_rectA && a_rectB);
...
template<class DATATYPE, class ELEMTYPE, int NUMDIMS, class ELEMTYPEREAL, int TMAXNODES, int TMINNODES>
void
RTree<DATATYPE, ELEMTYPE, NUMDIMS, ELEMTYPEREAL, TMAXNODES, TMINNODES>::ReInsert(Node *a_node, ListNode **a_listNode) {
...
template<class DATATYPE, class ELEMTYPE, int NUMDIMS, class ELEMTYPEREAL, int TMAXNODES, int TMINNODES>
bool RTree<DATATYPE, ELEMTYPE, NUMDIMS, ELEMTYPEREAL, TMAXNODES, TMINNODES>::Search(Node *a_node, Rect *a_rect,
                                                                                    int &a_foundCount,
                                                                                    std::function<bool(
                                                                                            const DATATYPE &)> callback) const {
    assert(a_node);
...
template<class DATATYPE, class ELEMTYPE, int NUMDIMS, class ELEMTYPEREAL, int TMAXNODES, int TMINNODES>
bool RTree<DATATYPE, ELEMTYPE, NUMDIMS, ELEMTYPEREAL, TMAXNODES, TMINNODES>::knnSearch(DATATYPE &target, int number,
                                                                                       std::function<bool(
                                                                                               const DATATYPE &)> callback) const {
    return knnSearch(m_root, target, number, callback);
}

template<class DATATYPE, class ELEMTYPE, int NUMDIMS, class ELEMTYPEREAL, int TMAXNODES, int TMINNODES>
bool RTree<DATATYPE, ELEMTYPE, NUMDIMS, ELEMTYPEREAL, TMAXNODES, TMINNODES>::knnSearch(Node *a_node, DATATYPE &target,
                                                                                       int number,
                                                                                       std::function<bool(
                                                                                               const DATATYPE &)> callback) const {
    // 如果是不含数据的虚点
    if (a_node->IsInternalNode()) {
        ...
        for (int index = 0; index < a_node->m_count; ++index) {
            //就搜寻他的子节点
            if (!Search(a_node->m_branch[index].m_child, number, callback)) {
                // The callback indicated to stop searching
                return false;
            }
        }
        ...
        // 如果所搜寻的节点 当前同分支  此分支下的所有节点就是true
        for (int index = 0; index < a_node->m_count; ++index) {
            //找到所搜寻节点的分支  将同一分支下的节点均返回
            DATATYPE &id = a_node->m_branch[index].m_data;
            if (id == target) {
                targetNode = a_node;
                break;
